...
import com.zmbdp.admin.service.timedtask.bloom.ResetBloomFilterJobHandler;
...
public class AppUserServiceImpl implements IAppUserService {
    ...
    private void refreshAppUserBloomFilter() {
        log.info("布隆过滤器初始化开始 -----");
        resetBloomFilterJobHandler.refreshAppUserBloomFilter();
        log.info("布隆过滤器初始化结束 -----");
    }
    ...
    public void edit(UserEditReqDTO userEditReqDTO) {
        // 根据 id 查询
        // 先查询布隆过滤器
        if (!bloomFilterService.mightContain(APP_USER_PREFIX + userEditReqDTO.getUserId())) {
    ...
    public AppUserDTO findById(Long userId) {
        // 对 userId 进行判空操作
        if (userId == null || !bloomFilterService.mightContain(APP_USER_PREFIX + userId)) {
            return null;
        }
        // 查询 appUser 对象
