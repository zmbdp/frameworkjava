...
[功能特性](#核心特性) • [快速开始](#-快速开始) • [文档](#13-文档和部署) • [问题反馈](https://github.com/zmbdp/frameworkjava/issues)
...
- 🔄 **幂等性控制**: 基于AOP的分布式幂等性解决方案，支持HTTP和MQ场景，针对高并发场景进行优化
- 📦 **模块化设计**: 清晰的模块划分，便于扩展和维护
...
项目实现了布隆过滤器 + Caffeine + Redis的三级缓存架构，有效提升系统性能和可用性。通过布隆过滤器防止缓存穿透，Caffeine本地缓存提供毫秒级访问速度，Redis分布式缓存保证多实例间数据一致性。支持复杂泛型类型缓存，CacheUtil工具类封装完整操作流程，使用简单。

> 📖 **详细使用说明**：请参考 [开发指南 - 扩展缓存功能](#扩展缓存功能)

### 2. Redis 复杂数据结构操作

RedisService 提供了对 Redis 各种数据结构的增强操作支持，包括 String、List、Set、ZSet、Hash 等类型，所有操作均支持复杂泛型嵌套结构，如 `Map<String, List<Set<T>>>` 等。

### 3. 安全认证机制

采用JWT无状态认证，结合Redis实现灵活的状态管理。支持网关层统一鉴权、用户信息加密存储、B端/C端用户分离认证、白名单配置、Token刷新注销等功能。

### 4. 微服务治理

完整的微服务治理方案，基于Nacos实现配置中心和服务发现，统一网关路由和限流保护，基于Feign实现声明式服务调用，统一异常处理和响应格式。

### 5. 完善的工具类库

提供丰富的工具类库，包括 BeanCopyUtil（复杂对象拷贝）、JsonUtil（JSON处理）、CacheUtil（三级缓存）、JwtUtil（JWT处理）、ExcelUtil（Excel导入导出）、MailUtil（邮件发送）、StreamUtil（流操作）、ValidatorUtil（数据校验）、ThreadUtil（线程管理）、FileUtil（文件处理）等。

### 6. 增强型布隆过滤器

多实现版本支持（安全版、快速版和Redis版），线程安全实现，支持精确计数和近似计数双重统计，负载率和误判率实时监控，配置化管理，定时任务自动维护。

### 7. 智能验证码系统

根据用户输入（手机号/邮箱）自动选择发送方式，采用策略模式设计便于扩展。支持短信验证码（阿里云）和邮件验证码双通道，邮件标题和内容模板支持动态配置，提供统一的验证码发送接口。

### 9. 业务模块

项目预置了完整的业务模块，开箱即用：
- **用户管理**：B端/C端用户管理，支持微信、手机号、邮箱登录
- **配置管理**：参数配置管理，支持键值查询和批量查询
- **地图服务**：城市列表、地点搜索、经纬度定位等功能
- **文件服务**：文件上传、OSS签名获取等功能
- **消息队列**：RabbitMQ集成，提供生产者和消费者示例
- **定时任务**：支持定时任务执行，如布隆过滤器自动刷新
- **异常处理**：统一异常处理机制，业务异常封装，完善的错误码体系

### 10. Excel导入导出功能

基于EasyExcel实现，支持大数值处理、单元格合并、数据校验等功能，提供完整的导入结果反馈机制。

> 📖 **详细使用说明**：请参考 [开发指南 - Excel导入导出](#excel导入导出)

### 11. 邮件发送功能

基于Hutool Mail + Jakarta Mail实现，支持文本/HTML格式、抄送（CC）、密送（BCC）、附件和内嵌图片（cid方式），支持自定义MailAccount或使用全局配置，线程安全的邮件账号配置。

### 12. 分布式幂等性控制

项目提供了完整的幂等性解决方案，基于AOP实现，支持HTTP请求和MQ消息两种场景。支持防重模式和强幂等模式，三态设计灵活配置，多种Token获取方式，业务失败自动重试机制，高并发场景下使用原子操作和退避策略确保稳定性。

> 📖 **详细使用说明**：请参考 [开发指南 - 幂等性控制](#幂等性控制) 和 [常见问题 - 幂等性相关](#-常见问题)
...
- **SDK文档**: 项目提供完整的JavaDoc文档，位于`javapro/javadoc`目录
- **容器化部署**: Docker Compose部署方案，完整的中间件支持
...
| `share-mysql-dev.yaml` | MySQL公共配置（数据源配置、HikariCP连接池优化配置等） |
...
使用三级缓存架构，有效防止缓存穿透。缓存流程：请求 → 布隆过滤器(判断是否存在) → Caffeine本地缓存 → Redis缓存 → 数据库。
...
基于EasyExcel实现，支持大数值处理（超过15位自动转换为字符串）、单元格合并（基于注解自动合并相同值）、数据校验（基于Jakarta Validation）等功能。
...
- **高并发优化**：使用Redis `INCR` 原子操作统计重试次数，避免并发竞态问题；采用指数退避策略（50ms → 100ms → 150ms，最大300ms），有效降低Redis和CPU压力
...
- **业务模块扩展**：基于`zmbdp-mstemplate`微服务模板快速创建新业务模块
- **缓存策略扩展**：支持自定义缓存过期策略、多级缓存降级策略
- **认证授权扩展**：可自定义权限验证规则，扩展第三方登录集成
- **消息队列扩展**：支持多种消息模式、死信队列、延迟消息等
- **线程池扩展**：支持按业务模块自定义线程池，动态参数调整
- **工具类扩展**：所有工具类均支持扩展和自定义

> 📖 **详细扩展指南**：请参考项目源码和JavaDoc文档
...
**注意**：重试次数统计使用Redis `INCR` 原子操作，确保高并发场景下的数据一致性。
...
**A**: 基于`zmbdp-mstemplate`微服务模板创建新模块，遵循Controller-Service-Mapper分层架构，使用统一响应格式和异常处理，集成到网关路由配置即可。

### Q8: 幂等性注解在高并发场景下是否稳定？

**A**: 是的，系统针对高并发场景进行了全面优化：
- **原子操作**：使用Redis `INCR` 命令确保重试次数统计的原子性，彻底解决并发竞态问题
- **退避策略**：采用指数退避等待机制（50ms → 100ms → 150ms，最大300ms），避免忙等导致Redis和CPU压力过大
- **过期时间优化**：仅在第一次 `INCR` 时设置过期时间，减少不必要的Redis操作
- **降级处理**：添加了操作失败时的降级处理机制，确保系统在极端情况下的稳定性
