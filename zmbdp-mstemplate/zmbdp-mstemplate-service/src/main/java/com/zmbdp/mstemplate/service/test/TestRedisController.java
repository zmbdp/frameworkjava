...
//        log.info(redisService.getCacheListByRange(key, -3, 0, String.class).toString()); // 获取 list 指定范围的元素
    ...
    public Result<Void> comprehensiveTest() {
        try {
            ...
            log.info("复杂泛型 Set<List<Map<String, RegionTest>>> 获取到 {} 个List元素", complexSet.size());

            // 遍历复杂数据结构
            for (List<Map<String, RegionTest>> list : complexSet) {
                log.info("  List包含 {} 个Map元素", list.size());
                for (Map<String, RegionTest> map : list) {
                    log.info("    Map包含 {} 个RegionTest元素", map.size());
            ...
            log.info("复杂泛型 Set<Map<String, List<RegionTest>>> 获取到 {} 个Map元素", complexSet2.size());
            ...
            for (Map<String, List<RegionTest>> map : complexSet2) {
                ...
                for (Map.Entry<String, List<RegionTest>> entry : map.entrySet()) {
                    log.info("    Key: {}, Value包含 {} 个RegionTest元素", entry.getKey(), entry.getValue().size());
            ...
            log.info("getCacheSet 获取到 {} 个RegionTest对象", setRegions.size());
            ...
            log.info("复杂泛型 ZSet<List<Map<String, RegionTest>>> (升序) 获取到 {} 个List元素", complexZSet.size());

            // 获取复杂泛型ZSet (降序)
            Set<List<Map<String, RegionTest>>> complexZSetDesc = redisService.getCacheZSetDesc(complexZSetKey,
                new TypeReference<LinkedHashSet<List<Map<String, RegionTest>>>>() {});
            log.info("复杂泛型 ZSet<List<Map<String, RegionTest>>> (降序) 获取到 {} 个List元素", complexZSetDesc.size());
            ...
            log.info("复杂泛型 ZSet<Map<String, List<RegionTest>>> (升序) 获取到 {} 个Map元素", complexZSet2.size());

            // 获取复杂泛型ZSet (降序)
            Set<Map<String, List<RegionTest>>> complexZSetDesc2 = redisService.getCacheZSetDesc(complexZSetKey2,
                new TypeReference<LinkedHashSet<Map<String, List<RegionTest>>>>() {});
            log.info("复杂泛型 ZSet<Map<String, List<RegionTest>>> (降序) 获取到 {} 个Map元素", complexZSetDesc2.size());
            ...
            log.info("getZSetRangeDesc 获取到 {} 个RegionTest对象", zsetRangeDesc.size());
            ...
            log.info("getZSetRangeDesc (空集合) 获取到 {} 个RegionTest对象", zsetRangeDescEmpty != null ? zsetRangeDescEmpty.size() : 0);
            ...
            log.info("getCacheZSetDesc 获取到 {} 个RegionTest对象", cacheZSetDesc.size());
            ...
            log.info("getCacheZSetDesc (空集合) 获取到 {} 个RegionTest对象", cacheZSetDescEmpty != null ? cacheZSetDescEmpty.size() : 0);
            ...
            log.info("getZSetRangeByScore (空集合) 获取到 {} 个RegionTest对象", zsetRangeByScoreEmpty != null ? zsetRangeByScoreEmpty.size() : 0);
            ...
            log.info("getZSetReverseRangeByScore 获取到 {} 个RegionTest对象", zsetReverseRangeByScore.size());
            ...
            log.info("getZSetReverseRangeByScore (空集合) 获取到 {} 个RegionTest对象", zsetReverseRangeByScoreEmpty != null ? zsetReverseRangeByScoreEmpty.size() : 0);
            ...
            List<RegionTest> multiComplexValues = redisService.getMultiCacheMapValue(simpleComplexHashKey, Arrays.asList("region1", "region2"), new TypeReference<List<RegionTest>>() {});
            log.info("复杂泛型 getMultiCacheMapValue 获取到 {} 个值", multiComplexValues.size());

            // getMultiCacheMapValue (复杂泛型，部分存在的字段)
            List<RegionTest> multiComplexValuesPartial = redisService.getMultiCacheMapValue(simpleComplexHashKey, Arrays.asList("region1", "region3"), new TypeReference<List<RegionTest>>() {});
