...
- `RedisSlidingWindowRateLimiter`：滑动窗口实现（ZSET + Lua，封装算法细节）
- `RateLimitKeyBuilder`：Key 构建器（集中管理维度逻辑，便于扩展）
- `RateLimitConfigResolver`：配置解析器（注解 > Nacos > 默认值）
- Redis：**滑动窗口**（ZSET + Lua），限流更平滑，无边界突发
...
| `ratelimit.default-limit` | 全局默认限流阈值 | `60` |
| `ratelimit.default-window-sec` | 全局默认时间窗口（秒） | `60` |
...
4. **原子性保证**：使用 Lua 脚本保证滑动窗口（ZREMRANGEBYSCORE + ZCARD + ZADD + EXPIRE）的原子性。
5. **降级策略**：Redis 异常时可配置 `fail-open`（失败放行）或 `fail-close`（失败拒绝）。生产环境建议 `false`（安全优先）。
6. **滑动窗口算法**：按「当前时刻往前 windowSec 秒」统计请求数，窗口随请求滑动，避免固定窗口在 59s/61s 等边界的突发流量。
